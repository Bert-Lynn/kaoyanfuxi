...
export const BASE = {start:'2026-09-24',xzAnchor:'2026-09-24',enAnchor:'2026-09-24',enStartYear:2025,redbookDone:9,bioMapsDone:29,ankiDone:0,hulusiBlock:'10',historyLeft:4,xiaoMarxDone:3,essayAlt:true,examDate:'',name:'',protectReserved:true};
...
export function generate(date,c){const out=[];if(!validDate(date)||date<c.start)return out;const n=Math.max(0,daysBetween(date,c.start));
...
export function tasksFor(store,date){return store.get('day',date)?store.all('task').filter(t=>t.date===date):generate(date,cfg(store))}
export function materialize(store,date){if(store.get('day',date))return;for(const t of generate(date,cfg(store)))store.put('task',t.id,t);store.put('day',date,{id:date})}
...
export function progress(store){const c=cfg(store);const sets={xz:new Set([2020,2021,2022,2023,2024,2025,2026]),maps:new Set(Array.from({length:c.bioMapsDone},(_,i)=>i+1)),words:new Set(Array.from({length:c.redbookDone},(_,i)=>i+1)),anki:new Set(Array.from({length:c.ankiDone},(_,i)=>i+1))};for(const t of store.all('task').filter(t=>t.done))for(const[k,a]of Object.entries(t.units||{}))if(sets[k]&&Array.isArray(a))a.forEach(v=>sets[k].add(v));return Object.fromEntries(Object.entries(sets).map(([k,v])=>[k,v.size]))}
